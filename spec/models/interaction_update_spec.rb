...
RSpec.describe MasterControl::Models::InteractionUpdate do
  NULLEABLE_ATTRIBUTES = [].freeze

  let(:canonical_klass) { 'MasterControl::Models::InteractionUpdate' }
  let(:master_control_version) { '1.0.0' }
  let(:is_sync_update) { true }
  let(:version) { 1 }
  let(:id) { SecureRandom.uuid }
  let(:description) { 'Billing' }
  ...
  def validate!
    JSON::Validator.validate!(MasterControl::Models::InteractionUpdate.json_schema, interaction_update)
  end
